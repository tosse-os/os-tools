    ...
    <a href="{{ route('results.show', $result) }}"
      ...
      <div>
        <div class="text-sm font-medium">
          {{ strtoupper($result->type) }} – {{ $result->url }}
        </div>
      ...
      <div class="flex items-center gap-6">
        ...
        <span class="px-2 py-1 text-xs rounded {{ $statusColors[$result->status] ?? 'bg-gray-100 text-gray-600' }}">
          {{ $result->status }}
        </span>
